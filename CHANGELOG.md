...
- round 429: **raster-scan slice layout end-to-end (`pps_rect_slice_flag = 0`)**. `EncoderConfig::raster_slice_layout` re-signals the tile grid as a raster layout: the PPS skips the single-slice flag / slice loop, and the slice header carries `sh_slice_address` (u(v), `Ceil(Log2(NumTilesInPic))` bits) + `sh_num_tiles_in_slice_minus1`; the decoder resolves the CTB plan through `TileScan::raster_slice_ctbs` (the §6.5.1 tile-run arm) instead of `CtbAddrInSlice`. The `tiles_2x2_raster_256x256` axis decodes byte-exactly through both receive paths (123 of 123 externally byte-exact) with a plane hash equal to the rectangular-layout axis — only the signalling differs, pinning that both arms produce the identical walk. Also: §8.5.2.3 merge-scan pins for the §6.4.4 WPP column cap (B0 gated, B1 unaffected) and the tile-rectangle arm at the `MotionField` level.
