...
- round 440: **corpus-triage debug aids** — `H266_DBG_TB=x,y` dumps the single-TB intra luma pipeline state (parsed levels, prediction, residual slices) for one CU plus a per-slice tool-state line, and `examples/sps_dump.rs` prints a stream's full SPS tool-flag set for corpus family diffing.
