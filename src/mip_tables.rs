// Auto-extracted from ITU-T H.266 (V4) (01/2026), §8.4.5.2.4 MIP weight
// matrix derivation process. The spec writes each matrix as the printed
// braced form `{ {row 0}, {row 1}, ... }`, where each row contains the
// `inSize` values for one output position j = y*predSize + x. We store
// the values exactly in that printed order: `MIP_W[mode][j][i]` so that
// `mWeight[i][j] = TABLE[mode][j][i]` per the spec's eq. (270) reference.
// Values are in [0, 127] so a `u8` suffices.

/// MIP weight tables for sizeId 0 (predSize=4, inSize=4) — 16 modes.
pub const MIP_W_SZ0: [[[u8; 4]; 16]; 16] = [
    // sizeId 0, modeId 0
    [
        [32, 30, 90, 28],
        [32, 32, 72, 28],
...
/// MIP weight tables for sizeId 1 (predSize=4, inSize=8) — 8 modes.
pub const MIP_W_SZ1: [[[u8; 8]; 16]; 8] = [
    // sizeId 1, modeId 0
    [
...
/// MIP weight tables for sizeId 2 (predSize=8, inSize=7) — 6 modes.
pub const MIP_W_SZ2: [[[u8; 7]; 64]; 6] = [
    // sizeId 2, modeId 0
    [
